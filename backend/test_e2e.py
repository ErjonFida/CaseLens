...
def login_user(email, password, user_agent="TestAgent"):
    ...
    res = requests.post(f"{BASE_URL}/api/login", json={"email": email, "password": password}, headers=headers)
    return res.status_code, res.json()
...
def upload_file(filename, content, token):
    ...
    try:
        with open(filename, "rb") as f:
            ...
            res = requests.post(f"{BASE_URL}/api/upload", files=files, headers=headers)
        return res.status_code, res.json()
    finally:
...
def check_status(filename, token):
    headers = {"Authorization": f"Bearer {token}"}
    res = requests.get(f"{BASE_URL}/api/status/{filename}", headers=headers)
    return res.status_code, res.json()

def run_query(query_text, token):
    headers = {"Authorization": f"Bearer {token}"}
    res = requests.post(f"{BASE_URL}/api/search", json={"query": query_text}, headers=headers, stream=True)
...
def run_chat(query_text, token):
    headers = {"Authorization": f"Bearer {token}"}
    res = requests.post(
        f"{BASE_URL}/api/chat",
        json={"messages": [{"role": "user", "content": query_text}]},
        headers=headers,
        stream=True
    )
    if res.status_code != 200:
...
def run_test():
    ...
    assert "Welcome to CaseLens" in new_email, "Welcome email subject mismatch"
    ...
    # Test login device fingerprinting alerts
    ...
    # Test multi-tenant directory & path isolation
    ...
    for filename, token in [(file_alice, token_alice), (file_bob, token_bob)]:
        ...
        assert state == "completed", f"Indexing did not complete for '{filename}': {state}"
    ...
    assert status == 200, f"Search failed for Alice: {answer_alice}"
    assert "ALICE_KEY_999" in answer_alice, "Alice cannot retrieve her own document"
    assert "BOB_KEY_777" not in answer_alice, "ISOLATION FAILURE: Bob's content reached Alice"
    ...
    assert status == 200, f"Search failed for Bob: {answer_bob}"
    assert "BOB_KEY_777" in answer_bob, "Bob cannot retrieve his own document"
    assert "ALICE_KEY_999" not in answer_bob, "ISOLATION FAILURE: Alice's content reached Bob"
    print("Bob query isolates database records successfully.")

    print("\n[5] Testing /api/chat streaming endpoint...")

    if os.environ.get("GEMINI_API_KEY"):
        chat_status, chat_answer = run_chat("What is Alice's reference key?", token_alice)
        print(f"Chat streaming result for Alice:\n{chat_answer}")
        assert chat_status == 200, f"Chat failed: {chat_answer}"
        assert chat_answer.strip(), "Chat returned an empty response"
    else:
        print("SKIPPED: GEMINI_API_KEY is not set, and /api/chat needs it to generate.")
